...
public class MessageService implements IMessageServiceLocal, IMessageServiceRemote {
	...
	private String addUser (UserDto dto, OtpAuth otpAuth) throws MessageServiceExn {
		...
		try {
			...
			for(String r : dto.getRoles()) {
				roleDAO.getRole(r).addUser(user);
	...
	public long addMessage(MessageDto dto) {
		...
		loggedInUser=dto.getSender();
		authorization(dto.getSender());
	...
	public void checkOtp(String username, Long otpCode) throws MessageServiceExn {
		...
		try {
			...
			String otp_new = user.getOtpSecret();
			long time = System.currentTimeMillis();
			validOtp = OneTimePassword.checkCode(otp_new, otpCode, time);
	...
	public void authorization(String usr){
		ArrayList<String> valid_poster = new ArrayList<String>();
		valid_poster.add("jane");
		valid_poster.add("harsh");
		valid_poster.add("john");
		if(!valid_poster.contains(usr)){
